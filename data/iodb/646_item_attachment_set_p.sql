...
BEGIN
    ...
    INSERT INTO {schema}.item_attachment(
        item_id, created_by, caption, storage_url, file_hash, created_date)
        VALUES ( insert_item_id, insert_user_id, insert_caption, insert_storage_url, insert_file_hash, inserted_date);

END; 
$BODY$
;

ALTER PROCEDURE {schema}.item_attachment_set(bigint, bigint, text, text, text, timestamp with time zone)
    OWNER TO postgres;